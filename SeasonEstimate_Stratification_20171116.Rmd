...
for (jj in 1:length(SiteList)){
     DF_temp=DF[DF_warm$Name==SiteList[jj],]
     for (iii in 1:16){     
     tryCatch({ 
...
#Then Cool Season
for (jj in 1:length(SiteList)){
     DF_temp=DF[DF_cool$Name==SiteList[jj],]
     for (iii in 1:16){     
     tryCatch({ 
          tempname=alllist[iii,2]
          for (kkk in 0:2){
...
Cardio = filter(tlnise.result3, Outcome == 'Cardio') %>% 
     ggplot(aes(x=Lag,y=Effect,color=Season)) +
     	...
     	geom_point(aes(),cex=3.25, position = position_dodge(width = 0.50))+
          scale_y_continuous(limits = c(0.97,1.07))+
