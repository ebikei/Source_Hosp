...
#     covariates=c('ExpoCool','ExpoWarm','DayIndex','Age','ns(meanapptemp,df.temp)','ns(day,df.day)')
     covariates=c('ExpoCool','ExpoWarm','DayIndex','Age','ns(meanapptemp,df.temp)','ns(coolday,16)','ns(warmday,16)')     
#      covariates=c('ExpoCool','ExpoWarm','DayIndex','Age','ns(meanapptemp,df.temp)','ns(coolday,knot = c(45, 420, 780, 1145,  1510, 1875, 2240, 2605))','ns(warmday, knot = c(210, 575,  940, 1305, 1670, 2035, 2400, 2765))') 
...
tlnise.result2$Z_Cool=tlnise.result2$PE_Cool/tlnise.result2$SE_Cool
tlnise.result2$P_Cool= 1 - 2*abs(0.5-(1-pnorm(tlnise.result2$Z_Cool))) 
...
tlnise.result2$Z_Warm=tlnise.result2$PE_Warm/tlnise.result2$SE_Warm
tlnise.result2$P_Warm= 1 - 2*abs(0.5-(1-pnorm(tlnise.result2$Z_Warm))) 
...
Cool = tlnise.result2 %>% 
     select(Outcome, Pollutant, Lag, IQR, Effect_Cool, LCI_Cool, UCI_Cool, P_Cool) %>%
     rename(PE=Effect_Cool, LCI=LCI_Cool, UCI=UCI_Cool, P=P_Cool) %>% 
     mutate(Season='Cool')
Warm = tlnise.result2 %>% 
     select(Outcome, Pollutant, Lag, IQR, Effect_Warm, LCI_Warm, UCI_Warm, P_Warm) %>%
     rename(PE=Effect_Warm, LCI=LCI_Warm, UCI=UCI_Warm, P=P_Warm) %>% 
     mutate(Season='Warm')

tlnise.result3 = rbind(Cool, Warm) %>% 
     mutate(Name=case_when(Pollutant == 'asalt'~ "Aged Sea Salt",
                                        ...
                                        LineCol = case_when(PE < 1 & P < 0.05 ~ 'blue',
                                                            PE < 1 & P < 0.10 ~ 'green',
                                                            PE > 1 & P < 0.05 ~ 'red',
                                                            PE > 1 & P < 0.10 ~ 'orange',
                                                            TRUE ~ 'black')) %>% 
...
Cardio = filter(tlnise.result3, Outcome == 'Cardio') %>% 
     ggplot(aes(x=Lag,y=PE,color=Season)) +
