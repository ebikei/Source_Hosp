...
DF %>% 
       group_by(Name) %>%
       do(data.frame(Cor=t(cor(.[,c(6:7,9:12)], .[,8], use = 'pairwise.complete.obs'))))

# Correlation between Sec.Sulfate and PM2.5
DF %>% 
       group_by(Name) %>%
       do(data.frame(Cor=t(cor(.[,c(6:7,9:12)], .[,6], use = 'pairwise.complete.obs'))))
...
#     covariates=c('DayIndex','Race','ns(meanapptemp,df.temp)', 'YEAR','Warm')     
...
#save(Result.1stall, file = 'K:\\Research\\PMSource_Hosp\\Data\\Result\\Result_RaceAdjust_CitySpecific.RData')
```
...
#save(tlnise.result2, file = 'K:\\Research\\PMSource_Hosp\\Data\\Result\\Result_RaceAdjust.RData')
```
...
```
Plot City Specific Results
```{r eval=FALSE, include=FALSE}
library(grid)
p = list()
for (mm in 1:12){
#mm = 7     
     Plot = filter(Result.1stall,Outcome==alllist2$Outcome[mm],Pollutant==alllist2$Pollutant[mm]) %>% 
          ...
          mutate(Effect = exp(estimate*IQR),
                 ...
                 LineCol = case_when(estimate < 0 & p.value < 0.05 ~ 'blue',
                                             ...
                                             TRUE ~ 'black')) %>% 
          ggplot(aes(x=Name,y=Effect,color=LineCol)) +
          	geom_errorbar(aes(ymin=LCI,ymax=UCI),size=1,width=0.35)+
          	geom_point(aes(),cex=3.25)+
               facet_grid(.~Lag)+
     	     geom_hline(yintercept=1,color='Black')+
               ylab("OR per IQR increase")+
               xlab("")+
               ggtitle(paste(alllist2$Outcome[mm],' related Hospital Admission and ', alllist2$Pollutant[mm], ' exposure', sep=''))+
               theme_bw()+
               scale_color_identity()+
               theme(legend.position="none", plot.title = element_text(hjust = 0.5), 
                axis.text.x = element_text(vjust =0.5, angle = 0), panel.grid.major = element_blank(),
...
test = marrangeGrob(grobs = p, nrow=2, ncol=1)

pdf("C:\\Users\\kebisu\\Downloads\\Hosp.pdf", onefile = TRUE,20,15)
grid.draw(test)
dev.off()
...
tldata = filter(Result.1stall,Outcome==alllist2$Outcome[mm],Pollutant==alllist2$Pollutant[mm], Lag ==alllist2$Lag[mm]) %>% 
     inner_join(Air, by='Pollutant') %>% 
     mutate(Effect = exp(estimate*IQR),
            LCI = exp((estimate-1.96*std.error)*IQR),
            UCI = exp((estimate+1.96*std.error)*IQR),               
...
prior=0
seed <- round(10000*runif(1))
tlnise.out=tlnise(tldata$estimate,tldata$std.error^2,prior=prior,maxiter=5000,seed=seed)
tlnise.out$gamma     
