...
DF.A=filter(Original_df2,Zone!='C') %>% 
     ...
     data.frame()
DF.A=DF.A[,c(1:7, 14:18, 25, 32:37)]
#DF.A=DF.A[,c(1:11,38,12:22,39,23:31,40,32:37)]

DF.A2 = DF.A %>% 
     select(Name, date,
            Cardio.White = cardio_disease_white, Cardio.Black = cardio_disease_black,
            Cardio.Asian = cardio_disease_asian, Cardio.Hispanic = cardio_disease_hispanic,
            Resp.White = resp_disease_white, Resp.Black = resp_disease_black,
            Resp.Asian = resp_disease_asian, Resp.Hispanic = resp_disease_hispanic,
            Pop.White = White, Pop.Black = Black, Pop.Asian = Asian, Pop.Hispanic = Hispanic)
#select(DF.A, Name, date) 

DF.A3 = reshape(DF.A2, dir = "long", varying = 3:14, sep = ".", idvar=c('Name', 'date')) %>% 
     arrange(Name, date) %>% 
     mutate(Race = as.factor(time)) %>% 
     select(Name, date, Race, Cardio, Resp, Pop)
DF.A3$Race = fct_relevel(DF.A3$Race, "White")
...
#     for (iii in 1:dim(alllist)[1]){
     ...
     tryCatch({ 
          ...
          for (kkk in 0:2){
               ...
               DF_temp2 = arrange(DF_temp1, Name, Race, date) %>% 
                    group_by(Name, Race) %>%
