...
Plot for very small species to check its distributions
```{r eval=FALSE, include=FALSE}
Rb = AirWeather2 %>% 
     select(date,Name, Rb) %>% 
     ggplot() +
     geom_point(aes(x = date, y = Rb)) +
...
pol_list = c("totpm25","secsulf","secnit","vehic","biomass","soil", "fsalt", "asalt")
alllist = filter (alllist, !(Exposure %in% pol_list))
#pol_list= c('Fe', 'In', 'NH4.','NO3.','SO42.')
#alllist = filter (alllist, Exposure %in% pol_list)
...
#save(tlnise.result2, file = 'K:\\Research\\PMSource_Hosp\\Data\\Result\\Result_RaceAdjust_Species_00to18.RData')
```
