...
DF = filter(Original_df2, Zone!='C') %>% 
     filter(Race == 'hispanic', Age == '00to18') %>% 
     group_by(Name,date, disease) %>%  #Add/Remove varible here in which level I want to analyze
...
#     covariates=c('DayIndex','ns(meanapptemp,df.temp)','ns(day,df.day)','Cool')
     covariates=c('DayIndex','ns(meanapptemp,df.temp)','ns(day,df.day)')
#     covariates=c('DayIndex', 'ns(meanapptemp,df.temp)','ns(day,df.day)', 'YEAR','Warm')        
...
#save(tlnise.result2, file = 'K:\\Research\\PMSource_Hosp\\Data\\Result\\Result_AgeRaceStrat_00to18_Hispanic.RData')
```
...
Plot City specific estimates

```{r}
CitySpecific = Result.1stall %>% 
     filter(Outcome == 'Resp', Pollutant == 'vehic') %>% 
