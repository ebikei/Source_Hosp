...
#Load O3 and NO2 Data
load('K:\\Research\\PMSource_Hosp\\Data\\O3NO2Data.RData') #df.O3NO2
...
AirWeather = Original_df2 %>% 
     ...
     left_join(df.O3NO2, by =c('Name', 'date')) %>%
...
DF %>% 
     ...
     summarize(PM25 = mean(totpm25, na.rm = TRUE), BioMass = mean(biomass, na.rm = TRUE), 
               ...
               O3 = mean(O3, na.rm = TRUE), NO2 = mean(NO2, na.rm = TRUE))
...
# Correlation between Sources and O3
DF %>% 
       group_by(Name) %>%
       do(data.frame(Cor=t(cor(.[,c(6:7,9:12)], .[,c(61)], use = 'pairwise.complete.obs'))))
# Correlation between Sources and NO2
DF %>% 
       group_by(Name) %>%
       do(data.frame(Cor=t(cor(.[,c(6:7,9:12)], .[,c(62)], use = 'pairwise.complete.obs'))))

# Correlation between Sec.Sulfate and PM2.5
DF %>% 
       group_by(Name) %>%
       do(data.frame(Cor=t(cor(.[,c(6:7,9:12,61:62)], .[,6], use = 'pairwise.complete.obs'))))
...
#     covariates=c('exposure_cool','exposure_warm','DayIndex','Race','ns(meanapptemp,df.temp)','ns(day,df.day)',  'Warm') 
     covariates=c('exposure','O3','DayIndex','Race','ns(meanapptemp,df.temp)','ns(day,df.day)')
...
tlnise.result2 = tlnise.result2 %>% 
          mutate(Variable2 = Pollutant) %>% 
        mutate(Variable2 = replace(Variable2, Variable =='O3', 'O3'))
#         mutate(Variable2 = replace(Variable2, Variable =='NO2', 'NO2'))     
```
...
tlnise.result3 = tlnise.result2 %>% 
     ...
     inner_join(Air, by = c('Variable2' = 'Pollutant')) %>% 
...
tlnise.result3 %>% filter(P<0.10) %>% arrange(Variable, PE) %>% select(Outcome, Pollutant, Lag, N , Variable, IQR, Effect, LCI, UCI, P)
```
...
tlnise.result4 = filter(tlnise.result4, Pollutant!='fsalt'& Pollutant!='asalt' & Variable == 'exposure')
tlnise.result4$Name[tlnise.result4$Name=='PM2.5'] = '"PM[2.5]"'
tlnise.result4$Name[tlnise.result4$Name=='O3'] = '"O[3]"'
tlnise.result4$Name[tlnise.result4$Name=='NO2'] = '"NO[2]"'
