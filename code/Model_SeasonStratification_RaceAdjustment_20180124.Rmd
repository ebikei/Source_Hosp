...
#     covariates=c('DayIndex','Race','ns(meanapptemp,df.temp)','ns(day,df.day)','YEAR','Warm')     
...
ptm=proc.time()
#Warm Season
Result.1st_warm=list()
for (iii in 1:dim(alllist)[1]){
     tryCatch({ 
     tempname=alllist[iii,2]
     DF_warm$target_val = DF_warm[,c(tempname)]
     out = DF_warm %>% 
          arrange(Name, date) %>%
          ...
          mutate(Lag = alllist[iii,3], Pollutant = tempname, Outcome = alllist[iii,1], Season = 'Warm')
          Result.1st_warm[[iii]] <- out
          rm(out, tempname)
     }, error=function(e){})           
}
...
#Cool Season
Result.1st_cool=list()
for (iii in 1:dim(alllist)[1]){
     tryCatch({ 
     tempname=alllist[iii,2]
     DF_cool$target_val = DF_cool[,c(tempname)]
     out = DF_cool %>% 
          arrange(Name, date) %>%
          filter(disease == alllist[iii,1]) %>% 
          group_by(Name, Race) %>% 
          ...
          unnest() %>% 
          filter(term == 'exposure') %>% 
          mutate(Lag = alllist[iii,3], Pollutant = tempname, Outcome = alllist[iii,1], Season = 'Cool')
          Result.1st_cool[[iii]] <- out
          rm(out, tempname)
     }, error=function(e){})           
}

proc.time() - ptm
Result.1stall_warm = do.call(rbind, Result.1st_warm)
Result.1stall_cool = do.call(rbind, Result.1st_cool)
Result.1stall = rbind(Result.1stall_warm, Result.1stall_cool)
...
Air= DF %>% 
     ...
     mutate(Season = case_when(variable == 'Summer'~ "Warm", variable == 'Winter' ~ 'Cool')) %>%
     filter(Pollutant != 'Cool') %>% 
     rename (IQR = value) %>% 
     select(Season, Pollutant, IQR)
...
alllist2=distinct(Result.1stall,Outcome,Pollutant,Lag,Season, keep_all=TRUE)

tlnise.result=data.frame()
for (mm in 1:dim(alllist2)[1]){
     temp2nd=filter(Result.1stall,Outcome==alllist2$Outcome[mm],Pollutant==alllist2$Pollutant[mm],Lag==alllist2$Lag[mm], Season == alllist2$Season[mm])
     set.seed(21)
     prior=0
     seed <- round(10000*runif(1))
     tlnise.out=tlnise(temp2nd$estimate,temp2nd$std.error^2,prior=prior,maxiter=5000,seed=seed)
     tlnise.out2=data.frame(cbind(as.character(alllist2$Outcome[mm]),as.character(alllist2$Pollutant[mm]),as.character(alllist2$Lag[mm]), as.character(alllist2$Season[mm]), dim(temp2nd)[1],as.numeric(as.character(tlnise.out$gamma[1])),as.numeric(as.character(tlnise.out$gamma[2]))))
...
tlnise.result2=inner_join(tlnise.result,Air,by=c('Pollutant','Season'))
...
Cardio = filter(tlnise.result3, Outcome == 'Cardio') %>% 
     ggplot(aes(x=Lag,y=Effect,color=Season)) +
     	...
     	geom_hline(yintercept=1,color='Black')+ 
          ...
          theme_bw()+
          scale_colour_manual(values=c("#00BFC4", "#F8766D"), name = 'Season', breaks=c('Cool', 'Warm'),labels=c('Cool','Warm'))+
